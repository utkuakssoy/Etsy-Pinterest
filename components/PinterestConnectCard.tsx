...
export function PinterestConnectCard({
  ...
  const accountConnected = isPinterestConnected() || connected;
    ...
    <section className="rounded-lg border border-neutral-900 bg-[#050505] p-5 shadow-sm">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <h2 className="mt-1 text-xl font-semibold text-neutral-100">Publishing account</h2>
          <p className="mt-2 text-sm leading-6 text-neutral-500">Connect once to load boards, save drafts, and publish approved posts.</p>
        </div>
        <Link
          ...
          aria-disabled={!credentialsReady}
        >
      ...
      {!credentialsReady && (
        <div className="mt-4 rounded-md border border-amber-900/60 bg-amber-950/20 p-4 text-sm text-amber-300">
          ...
          <p className="mt-2">Add your Client ID and Client Secret once to enable the browser login flow.</p>
        </div>
      )}

      {!credentialsReady && <PinterestCredentialsForm />}
